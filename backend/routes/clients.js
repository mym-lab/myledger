...
// ⚠️  PROTECTED: owner or admin only; BLOCKED if any transactions exist.
// Accounting integrity rule: once transactions are recorded, the business
// record must stay intact. To remove a business with no transactions, the
// owner may delete it. To permanently remove a business that has transactions,
// contact admin — admin can pass ?force=true after verifying a backup exists.
router.delete('/:id', noEncoder, (req, res, next) => {
  try {
    const client = rowToClient(stmtClientById.get(req.params.id));
    if (!client || !canAccess(client, req.userId))
      return res.status(404).json({ error: 'Client not found' });

    // Only the business owner or an admin may delete a client
    const isOwner = client.ownerId === req.userId;
    const isAdmin = req.userRole === 'admin';
    if (!isOwner && !isAdmin)
      return res.status(403).json({ error: 'Only the business owner or an admin can delete a business.' });

    // Block deletion if any transactions exist (voided or active)
    const txCount = db.prepare(
      'SELECT COUNT(*) as n FROM transactions WHERE client_id=?'
    ).get(req.params.id)?.n || 0;

    if (txCount > 0 && req.query.force !== 'true') {
      return res.status(409).json({
        error: `Cannot delete — this business has ${txCount} transaction record(s). Financial data must not be destroyed. Download a backup first, then contact admin to force-remove if needed.`,
        transactionCount: txCount,
        hint: 'GET /api/clients/:id/backup to download all data first.',
      });
    }

    // Admin force-delete (or zero-transaction delete)
    withTransaction(() => {
    ...
    res.json({ message: 'Client and all related data deleted', transactionsRemoved: txCount });
  } catch (err) { next(err); }
});
