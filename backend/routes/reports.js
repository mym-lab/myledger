...
// ── Cash Flow Forecast ────────────────────────────────────────────────────────
// GET /api/reports/cashflow-forecast?clientId=&days=30|60|90
router.get('/cashflow-forecast', requireTier('solo'), (req, res, next) => {
  try {
    const { clientId, days: daysParam = '90' } = req.query;
    if (!clientId) return res.status(400).json({ error: 'clientId required' });
    const { client, txns } = getClientAndTx(clientId, req.userId);
    if (!client) return res.status(404).json({ error: 'Client not found' });

    const days = Math.min(180, Math.max(7, parseInt(daysParam) || 90));

    // Dates
    const today = new Date();
    today.setHours(0, 0, 0, 0);
    const todayStr      = today.toISOString().slice(0, 10);
    const forecastEndDt = new Date(today);
    forecastEndDt.setDate(today.getDate() + days);

    // History window for recurring pattern (90 days back)
    const hist90Dt = new Date(today);
    hist90Dt.setDate(today.getDate() - 90);
    const hist90Str = hist90Dt.toISOString().slice(0, 10);

    // ── Opening balance: net cash from all historical txns ────────────────────
    const allIncome  = txns.filter(t => t.type === 'income');
    const allExpense = txns.filter(t => t.type === 'expense');
    const openingBalance = round(
      sum(allIncome, 'amount_net') - sum(allExpense, 'amount_net')
    );

    // ── AR inflows: sent/partial invoices with future due_date ────────────────
    const pendingInvoices = db.prepare(`
      SELECT id, invoice_number, customer_name, total, due_date
      FROM   invoices
      WHERE  client_id=? AND status IN ('sent','partial') AND due_date >= ?
    `).all(clientId, todayStr);

    // ── Recurring expense pattern: avg weekly from last 90 days ───────────────
    const recentExp      = allExpense.filter(t => t.createdAt >= hist90Str && t.createdAt <= todayStr + 'T23:59:59');
    const totalRecentExp = sum(recentExp, 'amount_net');
    const weeklyExpenseAvg = round(totalRecentExp / 13); // 91 days ≈ 13 weeks

    const catMap = {};
    recentExp.forEach(t => {
      const cat = t.category || 'Other';
      catMap[cat] = (catMap[cat] || 0) + (t.amount_net || 0);
    });
    const recurringByCategory = Object.entries(catMap)
      .map(([category, total]) => ({ category, weeklyAvg: round(total / 13) }))
      .sort((a, b) => b.weeklyAvg - a.weeklyAvg)
      .slice(0, 8);

    // ── BIR tax obligations in forecast window ────────────────────────────────
    const taxTypes = client.taxTypes || [];
    const taxObligations = [];

    // VAT payable estimate from last full month
    const lastMoStart = new Date(today.getFullYear(), today.getMonth() - 1, 1);
    const lastMoEnd   = new Date(today.getFullYear(), today.getMonth(), 0);
    const lmStr    = lastMoStart.toISOString().slice(0, 10);
    const lmEndStr = lastMoEnd.toISOString().slice(0, 10);
    const lmIncome  = allIncome.filter(t => t.createdAt >= lmStr && t.createdAt <= lmEndStr + 'T23:59:59');
    const lmExpense = allExpense.filter(t => t.createdAt >= lmStr && t.createdAt <= lmEndStr + 'T23:59:59');
    const outputVAT  = round(sum(lmIncome, 'amount_vat'));
    const inputVAT   = round(sum(lmExpense, 'amount_vat'));
    const vatPayable = round(Math.max(0, outputVAT - inputVAT));

    // EWT compensation estimate from payroll
    const employees = db.prepare(
      `SELECT basic_salary FROM employees WHERE client_id=? AND employment_status IN ('active','regular')`
    ).all(clientId);
    const monthlyWHT = round(employees.reduce((s, e) => {
      const basic   = e.basic_salary || 0;
      const taxable = Math.max(0, basic - 20833);
      return s + round(taxable * 0.20 / 12);
    }, 0));

    // Monthly filing deadlines
    const monthsAhead = Math.ceil(days / 30) + 2;
    for (let m = 0; m <= monthsAhead; m++) {
      const forMonth  = new Date(today.getFullYear(), today.getMonth() + m, 1);
      const prevMonth = new Date(today.getFullYear(), today.getMonth() + m - 1, 1);
      const prevLabel = prevMonth.toLocaleString('en-PH', { month: 'long', year: 'numeric' });

      if (taxTypes.includes('2550M')) {
        const due = new Date(forMonth.getFullYear(), forMonth.getMonth(), 20);
        if (due >= today && due <= forecastEndDt) {
          taxObligations.push({ form: '2550M', description: `VAT Return – ${prevLabel}`, dueDate: due.toISOString().slice(0, 10), estimatedAmount: vatPayable });
        }
      }
      if (taxTypes.includes('2551M')) {
        const due = new Date(forMonth.getFullYear(), forMonth.getMonth(), 20);
        if (due >= today && due <= forecastEndDt) {
          const ptBase = round(sum(lmIncome, 'amount_net'));
          taxObligations.push({ form: '2551M', description: `% Tax – ${prevLabel}`, dueDate: due.toISOString().slice(0, 10), estimatedAmount: round(ptBase * 0.03) });
        }
      }
      if (taxTypes.includes('1601C') && monthlyWHT > 0) {
        const due = new Date(forMonth.getFullYear(), forMonth.getMonth(), 10);
        if (due >= today && due <= forecastEndDt) {
          taxObligations.push({ form: '1601C', description: `WHT Compensation – ${prevLabel}`, dueDate: due.toISOString().slice(0, 10), estimatedAmount: monthlyWHT });
        }
      }
    }
    ...
    const quarterDefs = [
      { qLabel: 'Q1', dueMonth: 4,  dueDay: 20, nextYear: false },
      { qLabel: 'Q2', dueMonth: 7,  dueDay: 20, nextYear: false },
      { qLabel: 'Q3', dueMonth: 10, dueDay: 20, nextYear: false },
      { qLabel: 'Q4', dueMonth: 1,  dueDay: 20, nextYear: true  },
    ];
    quarterDefs.forEach(({ qLabel, dueMonth, dueDay, nextYear }) => {
      const dueYear = nextYear ? today.getFullYear() + 1 : today.getFullYear();
      const due     = new Date(dueYear, dueMonth - 1, dueDay);
      if (due < today || due > forecastEndDt) return;
      if (taxTypes.includes('2550Q'))  taxObligations.push({ form: '2550Q',  description: `VAT Quarterly – ${qLabel} ${dueYear}`, dueDate: due.toISOString().slice(0, 10), estimatedAmount: round(vatPayable  * 3) });
      if (taxTypes.includes('1601EQ')) taxObligations.push({ form: '1601EQ', description: `EWT Quarterly – ${qLabel} ${dueYear}`, dueDate: due.toISOString().slice(0, 10), estimatedAmount: round(monthlyWHT * 3) });
    });

    taxObligations.sort((a, b) => a.dueDate.localeCompare(b.dueDate));
    ...
    for (let w = 0; w < weekCount; w++) {
      ...
      wsDate.setDate(today.getDate() + w * 7);
      const weDate = new Date(wsDate);
      weDate.setDate(wsDate.getDate() + 6);
      if (weDate > forecastEndDt) weDate.setTime(forecastEndDt.getTime());

      const wsStr = wsDate.toISOString().slice(0, 10);
      const weStr = weDate.toISOString().slice(0, 10);

      const arInWeek  = pendingInvoices.filter(inv => inv.due_date >= wsStr && inv.due_date <= weStr);
      const arTotal   = round(arInWeek.reduce((s, inv) => s + (inv.total || 0), 0));
      const taxInWeek = taxObligations.filter(t => t.dueDate >= wsStr && t.dueDate <= weStr);
      const taxTotal  = round(taxInWeek.reduce((s, t) => s + (t.estimatedAmount || 0), 0));

      const inflows   = arTotal;
      const outflows  = weeklyExpenseAvg;
      const netChange = round(inflows - outflows - taxTotal);
      runningBalance  = round(runningBalance + netChange);
      ...
      weeks.push({
        ...
        inflows,
        outflows,
        taxObligations: taxTotal,
        taxDetails: taxInWeek,
        arDetails:  arInWeek.map(inv => ({ invoiceNumber: inv.invoice_number, customer: inv.customer_name, amount: inv.total, dueDate: inv.due_date })),
        netChange,
        runningBalance,
      });
    }
    ...
    // ── Period summaries ──────────────────────────────────────────────────────
    function periodSummary(nDays) {
      const nWeeks = Math.ceil(nDays / 7);
      const slice  = weeks.slice(0, nWeeks);
      const inflows  = round(slice.reduce((s, w) => s + w.inflows, 0));
      const outflows = round(slice.reduce((s, w) => s + w.outflows + w.taxObligations, 0));
      const endBal   = slice.length ? slice[slice.length - 1].runningBalance : openingBalance;
      return { inflows, outflows, net: round(inflows - outflows), endBalance: endBal };
    }
    ...
    res.json({
      ...
      days,
      weeks,
      summary: { '30': periodSummary(30), '60': periodSummary(60), '90': periodSummary(90) },
      assumptions: { weeklyExpenseAvg, recurringByCategory, historyDays: 90, arPending: pendingInvoices.length, taxCount: taxObligations.length },
    });
  } catch (err) { next(err); }
});
